// investigacionData.js

const investigacionData = [
  {
    id: "quienesSomosIDi",
    title: "Quiénes Somos",
    content: `
      <h4>La Dirección de Investigación, Desarrollo e Innovación (I+D+i)</h4>
      <p>
        La Dirección de Investigación, Desarrollo e Innovación del ISTS está conformada por un equipo multidisciplinario de docentes e investigadores comprometidos con la generación de conocimiento relevante y la búsqueda de soluciones innovadoras para los desafíos actuales.
      </p>
      <p>
        Nuestra misión es fomentar la investigación aplicada, el desarrollo tecnológico y la innovación, promoviendo la participación activa de estudiantes y fortaleciendo las capacidades investigativas de la institución.
      </p>
      <p>
        <em>(Aquí podrías añadir el equipo de investigación, coordinadores, etc. similar al modal de "Equipo de trabajo" de Vinculación)</em>
      </p>
    `,
  },
  {
    id: "modeloInvestigacionVinculacion",
    title: "Modelo de Investigación y Vinculación",
    content: `
      <h4>Modelo de Investigación y Vinculación</h4>
      <p>
        Este modelo describe la estrategia que integra la investigación con la vinculación con la sociedad. Explica cómo la investigación del ISTS se orienta a resolver problemas reales de la comunidad y cómo los resultados se transfieren para generar impacto social y productivo.
      </p>
      <p>
        Nuestro enfoque se basa en la co-creación de soluciones, la interdisciplinariedad y la sostenibilidad de los proyectos, asegurando que el conocimiento generado sea pertinente y aplicable.
      </p>
      <a
        href="/assets/modules/investigacion/modelo_investigacion_vinculacion.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Descargar Modelo
      </a>
    `,
  },
  {
    id: "normativaInvestigacion",
    title: "Normativa",
    content: `
      <h4>Normativa de Investigación</h4>
      <p>
        Accede a los reglamentos, políticas y procedimientos que rigen las actividades de investigación, desarrollo e innovación en el ISTS. Estos documentos aseguran la calidad, la ética y la transparencia en todos nuestros procesos investigativos.
      </p>
      <p>
        Es fundamental que toda la comunidad investigadora conozca y aplique esta normativa.
      </p>
      <a
        href="/assets/modules/investigacion/normativa_investigacion.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Descargar Normativa Vigente
      </a>
    `,
  },
  {
    id: "procesosInvestigacion",
    title: "Procesos de Investigación",
    content: `
      <h4>Procesos de Investigación</h4>
      <p>
        Descripción detallada de los procesos internos para la formulación, presentación, evaluación, ejecución y cierre de proyectos de investigación. Incluye flujos de trabajo, responsabilidades y cronogramas.
      </p>
      <p>
        Desde la concepción de una idea hasta la difusión de resultados, cada etapa está diseñada para optimizar la eficiencia y la calidad científica.
      </p>
      <a
        href="/assets/modules/investigacion/procesos_investigacion.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Descargar Guía de Procesos
      </a>
    `,
  },
  {
    id: "dominiosLineasInvestigacion",
    title: "Dominios Académicos y Líneas de Investigación Institucionales",
    content: `
      <h4>Dominios Académicos y Líneas de Investigación Institucionales</h4>
      <p>
        Conoce los dominios académicos estratégicos del ISTS y las líneas de investigación prioritarias que orientan la producción científica de la institución. Estas líneas reflejan nuestra experticia y las áreas de mayor impacto potencial.
      </p>
      <p>
        Nuestras líneas de investigación son el marco sobre el cual construimos conocimiento relevante y aplicable, alineado con las necesidades del desarrollo nacional y regional.
      </p>
      <a
        href="/assets/modules/investigacion/dominios_lineas_investigacion.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Dominios y Líneas
      </a>
    `,
  },
  {
    id: "proyectosInvestigacion",
    title: "Proyectos de Investigación e Innovación en el Tecnológico Superarse",
    content: `
      <h4>Proyectos de Investigación e Innovación</h4>
      <p>
        Explora un listado de los proyectos de investigación e innovación que se están llevando a cabo o que han sido completados en el ISTS. Cada proyecto incluye una breve descripción, sus objetivos y los resultados esperados o alcanzados.
      </p>
      <p>
        Estos proyectos son la materialización de nuestra labor investigativa y su potencial impacto.
      </p>
      <a
        href="/assets/modules/investigacion/catalogo_proyectos_investigacion.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Catálogo de Proyectos
      </a>
    `,
  },
  {
    id: "redesAcademicasCientificas",
    title: "Redes Académicas y Científicas de I+D",
    content: `
      <h4>Redes Académicas y Científicas de I+D</h4>
      <p>
        Conoce las redes académicas y científicas en las que el ISTS participa activamente. Estas alianzas estratégicas fortalecen la colaboración, el intercambio de conocimientos y la internacionalización de nuestra investigación.
      </p>
      <p>
        Trabajamos con instituciones de prestigio para potenciar el impacto de nuestra investigación.
      </p>
      <a
        href="/assets/modules/investigacion/redes_academicas.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Redes y Alianzas
      </a>
    `,
  },
  {
    isSection: true,
    title: "Eventos",
  },
  {
    id: "congresoTopografia2023",
    title: "Congreso de Topografía 2023",
    content: `
      <h4>Congreso de Topografía 2023</h4>
      <p>
        Información detallada sobre el Congreso de Topografía 2023: objetivos, ponentes destacados, temáticas abordadas, fechas y lugar. Puede incluir un resumen de las conclusiones o acceso a las memorias.
      </p>
      <p><strong>Fecha:</strong> [Fecha del Congreso]</p>
      <p><strong>Lugar:</strong> [Lugar del Congreso]</p>
      <p><strong>Ponentes principales:</strong> [Nombres de ponentes]</p>
      <a
        href="/assets/modules/investigacion/eventos/congreso_topografia_2023.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Memorias / Información
      </a>
    `,
  },
  {
    id: "seminarioEquino",
    title: "Seminario Equino",
    content: `
      <h4>Seminario Equino</h4>
      <p>
        Detalles sobre el Seminario Equino: fecha, temas tratados, expositores y público objetivo. Puede ser un resumen del evento pasado o la convocatoria para uno futuro.
      </p>
      <a
        href="/assets/modules/investigacion/eventos/seminario_equino.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Detalles del Seminario
      </a>
    `,
  },
  {
    id: "simposioAdministracion",
    title: "Simposio de Administración",
    content: `
      <h4>Simposio de Administración</h4>
      <p>
        Información del Simposio de Administración: agenda, temas clave, fecha, lugar y participantes. Ideal para compartir el impacto y los conocimientos generados.
      </p>
      <a
        href="/assets/modules/investigacion/eventos/simposio_administracion.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Agenda / Información
      </a>
    `,
  },
  {
    isSection: true,
    title: "Convocatorias",
  },
  {
    id: "planificacionGestionInvestigacion",
    title: "Planificación Gestión de Investigación PAO mayo - octubre 2025",
    content: `
      <h4>Planificación Gestión de Investigación PAO mayo - octubre 2025</h4>
      <p>
        Accede al plan de gestión de investigación para el periodo mayo - octubre 2025, que detalla los objetivos, actividades clave y recursos asignados para la investigación durante este semestre.
      </p>
      <p>
        Conoce las directrices y el enfoque de nuestra gestión para este PAO.
      </p>
      <a
        href="/assets/modules/investigacion/convocatorias/planificacion_gestion_may_oct_2025.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Descargar Planificación
      </a>
    `,
  },
  {
    id: "proyectosInvestigacionPAO",
    title: "Proyectos de Investigación PAO mayo - octubre 2025",
    content: `
      <h4>Proyectos de Investigación PAO mayo - octubre 2025</h4>
      <p>
        Consulta la lista de proyectos de investigación aprobados o en desarrollo para el periodo académico ordinario (PAO) de mayo - octubre 2025, incluyendo sus resúmenes y los equipos de investigación involucrados.
      </p>
      <p>
        Descubre los temas de investigación que marcan la pauta para este semestre.
      </p>
      <a
        href="/assets/modules/investigacion/convocatorias/proyectos_investigacion_may_oct_2025.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Proyectos PAO
      </a>
    `,
  },
  {
    isSection: true,
    title: "Publicaciones",
  },
  {
    id: "publicacionesMayoOctubre2024",
    title: "Mayo - Octubre 2024",
    content: `
      <h4>Publicaciones Mayo - Octubre 2024</h4>
      <p>
        Listado de artículos científicos, libros, capítulos de libro y otros productos de investigación publicados por la comunidad del ISTS durante el periodo de mayo a octubre de 2024.
      </p>
      <p>
        Accede a la producción académica más reciente de nuestros investigadores.
      </p>
      <a
        href="/assets/modules/investigacion/publicaciones/publicaciones_may_oct_2024.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Publicaciones
      </a>
    `,
  },
  {
    id: "publicacionesNoviembre2023Abril2024",
    title: "Noviembre 2023 - Abril 2024",
    content: `
      <h4>Publicaciones Noviembre 2023 - Abril 2024</h4>
      <p>
        Listado de artículos científicos, libros, capítulos de libro y otros productos de investigación publicados por la comunidad del ISTS durante el periodo de noviembre 2023 a abril de 2024.
      </p>
      <a
        href="/assets/modules/investigacion/publicaciones/publicaciones_nov_2023_abr_2024.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Publicaciones
      </a>
    `,
  },
  {
    id: "publicacionesMayoOctubre2023",
    title: "Mayo - Octubre 2023",
    content: `
      <h4>Publicaciones Mayo - Octubre 2023</h4>
      <p>
        Listado de artículos científicos, libros, capítulos de libro y otros productos de investigación publicados por la comunidad del ISTS durante el periodo de mayo a octubre de 2023.
      </p>
      <a
        href="/assets/modules/investigacion/publicaciones/publicaciones_may_oct_2023.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Publicaciones
      </a>
    `,
  },
  {
    id: "publicacionesNoviembre2022Abril2023",
    title: "Noviembre 2022 - Abril 2023",
    content: `
      <h4>Publicaciones Noviembre 2022 - Abril 2023</h4>
      <p>
        Listado de artículos científicos, libros, capítulos de libro y otros productos de investigación publicados por la comunidad del ISTS durante el periodo de noviembre 2022 a abril de 2023.
      </p>
      <a
        href="/assets/modules/investigacion/publicaciones/publicaciones_nov_2022_abr_2023.pdf"
        target="_blank"
        class="btn btn-sm btn-info mt-3"
      >
        <i class="fa fa-file-pdf mr-2"></i> Ver Publicaciones
      </a>
    `,
  },
];